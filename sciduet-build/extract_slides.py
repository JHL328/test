...
def merge_titles(json_in):
    ...
    for i in json_in:
        counter = 0
        json_tmp[i] = {}
        if len(df) > 0 and (df['ID'] == str(i)).any():
            # Get the index of the first row that matches the condition
            idx = df.index[df['ID'] == str(i)][0]
            # Get the title of the paper using the index
            paper_title = df.loc[idx, 'Title']
            # Do something with the paper title
            print(paper_title)
        else:
            # Handle the case where the dataframe is empty or the condition is incorrect
            print('No paper found for ID:', i)
...
def random_forest(json_in):
    ...
    for i in data:
        file_path = 'paper_jsons/{}.json'.format(i)
